...
5. DPA 분석 과정 및 실제 파형 검증 상세 설명

DPA 분석 과정 (차분 전력 분석의 핵심 단계)
DPA는 수집된 방대한 전력 파형 데이터를 통계적으로 처리하여 올바른 비밀키를 찾아내는 과정입니다.

그룹 분류 (중간값 예측 및 비트 분할):

수집한 n개의 평문 세트와 공격자가 임의로 추측한 부분 키(Subkey)를 이용하여 내부 연산의 중간값(예: S-Box 출력값)을 계산합니다.

계산된 중간값의 특정 타겟 비트(주로 최상위 비트인 MSB)를 기준으로 삼습니다.

해당 비트 연산 결과가 '1'로 예측되는 평문들의 실제 측정 파형을 그룹 1로 모으고, '0'으로 예측되는 평문들의 파형을 그룹 0으로 나눕니다.

평균 파형 도출 및 차분(Difference) 계산:

그룹 1에 속한 파형들의 시간대별 평균을 구하고, 동일하게 그룹 0에 속한 파형들의 평균을 구합니다.

두 그룹의 평균 파형을 서로 뺍니다(차분 연산).

올바른 키 판별 (스파이크 확인):

잘못된 키를 추측했을 경우: 예측이 틀렸기 때문에 파형들이 그룹 1과 0에 무작위로 섞이게 됩니다. 결과적으로 두 그룹의 평균 파형 차이는 상쇄되어 거의 0에 수렴하는 평탄한 그래프가 나옵니다.

올바른 키를 추측했을 경우: 예측한 비트 값이 실제 기기 내부에서 처리된 비트 값과 정확히 일치합니다. 따라서 해당 데이터가 처리되는 정확한 시간대에서 두 그룹 간의 전력 소비량 차이가 두드러지게 나타나며, 그래프 상에 명확한 스파이크(피크)가 발생합니다.

실제 파형을 기준으로 한 점검 및 확인의 중요성

이론적인 통계 모델 계산만으로는 분석에 성공할 수 없습니다. 실제 환경에서는 주변 전자기파 노이즈, 기기 클럭의 미세한 변동으로 인한 파형 비동기화(Misalignment), 그리고 하드웨어 자체의 보안 대책 등으로 인해 파형이 훼손되거나 밀릴 수 있습니다.

따라서 반드시 수집된 실제 측정 파형의 정렬 상태와 노이즈를 눈으로 확인하고 전처리(필터링, 파형 정렬)해야 하며, 도출된 통계적 피크가 암호 알고리즘의 실제 연산 시점과 일치하는지 교차 검증하는 과정이 필수적입니다.
