...
void M_MSbox(byte m, byte m_prime) {

	for (int byte_idx = 0; byte_idx < 256; byte_idx++) {
		MSbox[byte_idx ^ m] = sbox[byte_idx] ^ m_prime;
	}
}
...
void M_SubBytes(byte state[16]) {//값을 변경하지 않고 MSbox를 불러옴

	for (int byte_idx = 0; byte_idx < AES_BLOCK_SIZE; byte_idx++)
	{
		state[byte_idx] = MSbox[state[byte_idx]]; // S-Box를 사용하여 바이트 치환
	}
}
...
int main() {
	...
	srand((unsigned int)time(NULL)); //1회 실행
